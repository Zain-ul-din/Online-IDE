...
export default function IDE() {
  ...
  useEffect(() => {
    const editorWindowMouseDown = (e: MouseEvent) => {
      ...
      const topWindowRect =
        editorTopWindowRef.current?.getBoundingClientRect() as DOMRect;
    ...
    editorHandleRef.current?.addEventListener(
      "mousedown",
      editorWindowMouseDown
    );
    ...
    return () => {
      ...
      editorHandleRef.current?.removeEventListener(
        "mousedown",
        editorWindowMouseDown
      );
    };
  ...
  return (
    <Flex w="100%" h="100%" flex={1} ref={parentRef} maxH={"100%"}>
      <Flex
        w={leftWinWidth}
        ref={leftWindowRef}
        ...
        flexDir={"column"}
      >
        <Flex w={"100%"} height={editorWinHeight} ref={editorTopWindowRef}>
          <Editor
            height={"100%"}
      ...
      <Flex h="100%" w="4px" bg="gray.200" cursor="col-resize" ref={handleRef}>
        {" "}
      </Flex>
      <Flex
        flex={1}
